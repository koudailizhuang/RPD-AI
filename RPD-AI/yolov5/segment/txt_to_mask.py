...
def txtToImg(path,savepath):
    ...
    for file_abs in files:
        ...
        print(file_abs[42:-4])  # 每个txt文件的名称
        # 按行读取txt文件
        f = open(file_abs)
        ...
        #print(file_abs[66:-4])
        img = cv2.imread(path+"\\"+file_abs[42:-4]+'.JPG')
        Img = 255*np.ones((200, 640), dtype=np.uint8)
        ...
        for i in range(len(list)):
            ...
            #print(mask)
            Img = cv2.polylines(img2, [mask], True, color,1)  # 将坐标点连接起来，线段颜色是0
            #cv2.circle(Img, [mask], 0.01, (0, 0, 255), 3)
        filepath = savepath + "\\" + file_abs[42:-4] + ".JPG"
        print(file_abs[42:-4])
        ...
        while line:
            ...
            filepath = savepath + "\\" + file_abs[52:-4] + ".jpg"
            cv2.imwrite(filepath, a)  # 将生成图片a存入路径。
            line = f.readline()
        '''


txtToImg(r"D:\algorithm\yolov5-v7.0-1\data\yolov5_nc",r"D:\algorithm\yolov5-v7.0-1\data\yolov5_nc_output")
